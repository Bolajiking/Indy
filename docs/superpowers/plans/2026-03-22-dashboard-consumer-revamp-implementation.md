...
describe("buildDashboardHomeModel", () => {
  ...
  it("returns calm empty states when there is no creator activity yet", () => {
    ...
    expect(model.supportRail.opportunities[0]?.title).toContain(
...
```ts
const { data, error, isLoading } = useAuthedQuery(
  fetchDashboardHome,
  EMPTY_HOME_DATA,
);
...
describe("consumer dashboard copy", () => {
  ...
  it("keeps wallet pending language calm and action-oriented", () => {
    expect(
      getWalletPendingCopy({ inProgress: true, lastError: null }),
...
export function getSignedOutCopy() {
  return {
    ...
    detail:
      "Use Privy to access your deals, messages, wallet activity, and approvals.",
...
```css
.cta-primary {
  @apply rounded-full bg-ink px-5 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-paper transition hover:bg-plum;
}
.cta-secondary {
  @apply rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-semibold uppercase tracking-[0.18em] text-ink transition hover:bg-parchment;
}
.surface-card {
  @apply rounded-[28px] border border-black/10 bg-white/75 shadow-card;
}
.surface-muted {
  @apply rounded-[22px] border border-black/10 bg-parchment;
}
```
